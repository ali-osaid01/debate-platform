          ...
          <ScrollArea className="h-[calc(100vh-8rem)] pr-4">
            <div className="space-y-4 py-4">
              {data?.response?.data?.data.map((notification:INotification) => (
                <div key={notification._id} className="flex space-x-4 items-start rounded-lg border p-4 hover:bg-gray-100 transition ease-in-out duration-150">
                  <Avatar>
                  ...
                  <div className="flex items-center text-xs text-muted-foreground gap-1 whitespace-nowrap">
                    <span title={new Date().toLocaleString()}>{formatShortDuration(notification.createdAt)}</span>
                  </div>
                </div>
              ))}
            </div>
